...
if ($found_conf_path) {
    // Lire le fichier conf.php pour récupérer le mot de passe
    ...
    // Récupérer aussi les autres paramètres si nécessaire
...
// Fallback : essayer depuis le fichier conf.php si les paramètres sont incomplets
if (empty($db_name) || empty($db_user) || !isset($conf->db->pass)) {
    $conf_file = DOL_DOCUMENT_ROOT.'/../conf/conf.php';
    if (file_exists($conf_file)) {
        // Sauvegarder les variables actuelles pour éviter les conflits
...
if ($action == 'install_complete') {
    try {
        ...
        if (empty($db_name) || empty($db_user)) {
            throw new Exception("Paramètres de base de données manquants. Host: $db_host, DB: $db_name, User: $db_user");
        }
